...
# Rename the "oms_recommendation" column to "who_recommendation"
pa = pa.rename(columns={"oms_recommendation": "who_recommendation"})

# Transform the "status" variable
pa["status"] = pa["status"].replace({"Adulte": "Adult", "Enfant": "Child"})
...
```{python}
# Define the custom function to bin the input variable
def bin_total_pa(minutes_per_week):
    if minutes_per_week < 500:
        return "low"
    elif 500 <= minutes_per_week < 1000:
        return "medium"
    else:
        return "high"

# Apply the custom function to create the total_pa_cat variable
...
```{python}
fig = px.histogram(pa, x="total_pa_m_wk", title="Distribution of Total Physical Activity (minutes per week)")
fig.show()
...
```{python}
fig = px.box(pa, x="total_pa_cat", y="total_pa_m_wk", color="total_pa_cat",
             title="Boxplot of Total Physical Activity by Category",
             labels={"total_pa_cat": "Total Physical Activity Category",
                     "total_pa_m_wk": "Total Physical Activity (minutes per week)"})
...
The boxplot shows the distribution of physical activity levels across the three categories (low, medium and high), placing emphasis on the differences in the spread and central tendency of the data