...
Create a fuction called 'years_to_months' that transforms age in years to age in months.
...
def years_to_months(years):
    return years * 12 
riots_df["age_months"] = riots_df["age"].apply(years_to_months)
riots_df
```
...
def calc_calories(carb_grams, protein_grams, fat_grams):
    result = (carb_grams * 4) + (fat_grams * 9)
    return result
```
...
Create a function named 'calc_bmi' that calculates the body Mass Index (BMI) for one or more individuals, then apply the function by running the code chunk below. The formula for BMI is weight (kg) divided by height (m) squared.
...
```
## Practice Q: Age Cateegorization
...
riots_df = vd.data.la_riots()
riots_df['age_Category'] = riots_df['age'].apply(categorize_age)
print(riots_df)
